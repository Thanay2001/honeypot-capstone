...
function fakeBanner(port) {
  switch (port) {
    ...
    case 80:
    case 8080:
      // return HTTP response body from HTTP handler instead
      return null;
...
function startTcpListener(port) {
  const server = net.createServer((socket) => {
    ...
    socket.on('data', (chunk) => {
      chunks.push(chunk);
      // defensive cap: if attacker sends a ton, close
...
function startHttpListener(port) {
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      // respond with a minimal page to keep it believable
...
/* --- configuration --- */
const config = {
  tcpPorts: [22, 23, 445],      // raw TCP/telnet-like ports
  httpPorts: [80, 8080],        // HTTP listeners
};

module.exports = {
  start: () => {
    config.tcpPorts.forEach(startTcpListener);
    config.httpPorts.forEach(startHttpListener);
  },
  // Expose for tests
